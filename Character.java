...
public class Character {
    ...
    public Character(double x, double y) throws IOException {
        ...
        this.runningX = 0;
        this.runningY = 0;
        this.characterHealth = 1000000;
        this.canHit = false;
        this.startGame = false;
        this.gameOverImage = ImageIO.read(new File("C:\\Users\\zarez\\IdeaProjects\\MENU_FIRST\\src\\failed.png"));
        this.winImage = ImageIO.read(new File("C:\\Users\\zarez\\IdeaProjects\\MENU_FIRST\\src\\passed.png"));
        this.isMoving = false;
    ...
    public void drawCoins(Graphics g) {
        if (startGame) {
            g.setFont(font1);
            g.fillRect(241, 0, 200, 48);
            g.setColor(Color.yellow);
            g.drawRect(241, 0, 200, 48);
            g.drawString("COINS:", 251, 35);
    ...
    public void drawGameOver(Graphics g) {
        if (startGame) {
            ...
            if (win) {
                g.drawImage(this.winImage, 0, 50, null);
                xRunningSpeed = 0;
                yRunningSpeed = 0;
                gameOver = true;
            }
            ...
            if (maxBalls > 0) {
                if (e.getButton() == 3 && e.getID() == MouseEvent.MOUSE_CLICKED) {
                    if ((currentTime - previousWorldUpdateTime) >= 1000) {
                        ...
                        balls.add(new FireBall(this, e.getX(), e.getY(), x + 20, y + 20, balls.size(), 0, 10));
